...
fun MissionGeneralScreen(
    ...
    Scaffold(
        ...
        topBar = {
            Column {
                GeneralMissionsHeader()
                NavigationRow(navController = navController, isWeeklyActive = false)
            }
        }
    ) { paddingValues ->
        when {
            ...
            uiState.generalMissions.isEmpty() -> {
                EmptyGeneralMissionsState(
                ...
                GeneralMissionsList(
                    missions = uiState.generalMissions,
                    onClaimClick = { missionId ->
                        viewModel.claimGeneralMissionReward(missionId)
                    },
                    modifier = Modifier.padding(paddingValues)
                )
            }
...
@Composable
private fun GeneralMissionsHeader() {
    Card(
        modifier = Modifier
            .fillMaxWidth()
            .padding(horizontal = 16.dp, vertical = 8.dp),
        ...
        Row(
            ...
            Icon(
                ...
                contentDescription = "General Missions",
...
@Composable
private fun GeneralMissionsList(
    missions: List<Pair<com.example.myapplication.data.database.Mission, com.example.myapplication.data.database.GeneralMissionUser>>,
    ...
    LazyColumn(
        ...
        items(missions) { (mission, missionUser) ->
            MissionCardGeneral(
                mission = mission,
                generalMissionUser = missionUser,
...
@Composable
private fun EmptyGeneralMissionsState(
    onRefresh: () -> Unit,
    modifier: Modifier = Modifier
) {
    Box(
        ...
        Column(
            ...
            Spacer(modifier = Modifier.height(16.dp))
            Text(
                text = "Nessuna missione generale",
            ...
            Text(
                text = "Le tue missioni generali appariranno qui quando sarai registrato e avrai selezionato i tuoi interessi.",
                style = MaterialTheme.typography.bodyMedium,
                color = MaterialTheme.colorScheme.onSurface.copy(alpha = 0.7f),
                textAlign = TextAlign.Center
            )
